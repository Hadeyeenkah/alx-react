...
describe('App Component Tests', () => {
  beforeEach(() => {
    // Set up the initial state for the mock Redux store
    store = mockStore({
      uiReducer: fromJS({
  ...
  it('should map isLoggedIn from Redux state correctly', () => {
    // Set up a store with the user logged in
    store = mockStore({
      uiReducer: fromJS({
        isUserLoggedIn: true,
        isNotificationDrawerVisible: false,
    ...
    // Check that the Redux state is passed correctly to the component
    const stateProps = wrapper.props().store.getState().uiReducer;
    expect(wrapper.props().store.getState().uiReducer.get('isUserLoggedIn')).toEqual(true);
  });

  it('should map displayDrawer from Redux state correctly', () => {
    // Set up a store with the drawer visible
    store = mockStore({
      uiReducer: fromJS({
        isUserLoggedIn: false,
    ...
    // Check that the Redux state is passed correctly to the component
    const stateProps = wrapper.props().store.getState().uiReducer;
    expect(wrapper.props().store.getState().uiReducer.get('isNotificationDrawerVisible')).toEqual(true);
  });